...
static void
handle_connection(struct sockmap_bpf *skel, int fd, int *split_fd)
{
	struct pollfd poll_fd;
	int rc, i, errsv, map_off = 0;

	/* Update the element with the input fd to attach the parse/verdict progs to that fd */
	rc = bpf_map_update_elem(bpf_map__fd(skel->maps.sock_map), &map_off, &fd, BPF_ANY);
	if (rc != 0) {
		errsv = errno;
...
{
	struct sockmap_bpf *skel;
	struct pollfd poll_fd;
	struct addrinfo hints = { 0 }, *ai;
	int i, client_fd[3], num_clients = 0;
	int rc = 0, fd = -1;
	...
	skel = sockmap_bpf__open();
	if (!skel) {
		fprintf(stderr, "Failed to open BPF skeleton\n");
		return 1;
	}

	rc = sockmap_bpf__load(skel);
	if (rc != 0) {
		fprintf(stderr, "Failed to load and verify BPF skeleton\n");
		goto cleanup;
	}

	rc = sockmap_bpf__attach(skel);
	if (rc != 0) {
	...
	fd = init_socket(AF_INET, ai->ai_addr, ai->ai_addrlen);
	if (fd < 0) {
		goto cleanup;
	}
